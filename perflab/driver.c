...
static void create(int dim)
{
	...
	for (i = 0; i < dim; i++) {
		for (j = 0; j < dim; j++) {
			...
			orig[RIDX(i, j, dim)].red =
				random_in_interval(0, 65536);
			orig[RIDX(i, j, dim)].green =
				random_in_interval(0, 65536);
			orig[RIDX(i, j, dim)].blue =
				random_in_interval(0, 65536);

			/* Copy of original image for checking result */
			copy_of_orig[RIDX(i, j, dim)].red =
				orig[RIDX(i, j, dim)].red;
			copy_of_orig[RIDX(i, j, dim)].green =
				orig[RIDX(i, j, dim)].green;
			copy_of_orig[RIDX(i, j, dim)].blue =
				orig[RIDX(i, j, dim)].blue;
...
{
	return (p1.red != p2.red) || (p1.green != p2.green) ||
	       (p1.blue != p2.blue);
}
...
static int check_rotate(int dim)
{
	...
	if (err) {
		...
		printf("src[%d][%d].{red,green,blue} = {%d,%d,%d}\n", badi,
		       badj, orig_bad.red, orig_bad.green, orig_bad.blue);
		printf("dst[%d][%d].{red,green,blue} = {%d,%d,%d}\n",
		       (dim - 1 - badj), badi, res_bad.red, res_bad.green,
		       res_bad.blue);
	}
...
void test_rotate(int bench_index)
{
	...
	for (test_num = 0; test_num < DIM_CNT; test_num++) {
		...
		/* Measure CPE */
		{
...
#endif
			arglist[0] =
				(void *)benchmarks_rotate[bench_index].tfunct;
			...
			create(dim);
			num_cycles =
				fcyc_v((test_funct_v)&func_wrapper, arglist);
			cpe = num_cycles / work;
...
#endif
			arglist[0] =
				(void *)benchmarks_smooth[bench_index].tfunct;
			arglist[1] = (void *)&tmpdim;
			arglist[2] = (void *)orig;
			arglist[3] = (void *)result;
